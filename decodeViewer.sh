...
BEGIN {
	...
	EXEC[i++]="sen_vars2";
	EXEC[i++]="ap_sys";
	# Local extension: row 48 carries the decoder exceptional-
	# path annotation (e.g. MIDFLOW, TRUNCATED). Empty for the
	# common path; "-" when the row only has 47 fields (upstream
	# recvQ schema that predates this extension).
	EXEC[i++]="decoder_note";
	#EXEC[i++]="country";
	#EXEC[i++]="region";
	#EXEC[i++]="city";
...
#print
	printf("\n\n");
	# EXEC tolerates two field counts:
	#   EXEC_CNT   (48) - extended schema with decoder_note
	#   EXEC_CNT-1 (47) - upstream recvQ schema (no decoder_note)
	# Both are valid; the print loop below substitutes "-" for
	# the missing field 48 on 47-field rows.
	if($2=="EXEC" && NF!=EXEC_CNT && NF!=EXEC_CNT-1){
		printf("\n\n@@ERROR== INCORRECT ================\n");
	...
	printf("%s--------------------------\n",$2);
	result="";
	# Iterate the schema length (not NF) so missing trailing
	# fields surface as "-" rather than silently disappearing.
	loop_end = NF;
	if ($2=="EXEC") loop_end = EXEC_CNT;
	for(i=1;i<=loop_end;i++){
		field=getVal($2,i);
		value = (i<=NF) ? $i : "-";
		result=fieldChk(field,value);
		if(result!=""){
			result=sprintf(" INCORRECT!  src %s:%s",client_ip,client_port);
		}
		printf("%s\t%3d <%s> --> <%s>%s\n",$2,i,field,value,result);
	}
}
'
